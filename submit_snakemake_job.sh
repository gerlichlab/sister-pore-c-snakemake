...
nohup snakemake --scheduler greedy --use-conda --use-singularity --rerun-incomplete --cluster "sbatch -t {cluster.time} -c {cluster.nodes} --mem {cluster.memory} -o slurm-out/%j.out"\
