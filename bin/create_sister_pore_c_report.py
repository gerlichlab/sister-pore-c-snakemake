...
def plot_brdu_distribution(thy_frame, ax):
    for sample in set(thy_frame.sample_name):
        ax.hist(
            thy_frame.query("sample_name == @sample").prob_brdu,
    ...
    sbn.despine()
    ax.set(
        title="BrdU probability distribution",
        xlabel="BrdU probability",
        ylabel="Density",
    )
...
def plot_brdu_per_thymidine_distribution(thy_frame, ax):
    ...
    brdu_per_read = (
        thy_frame_filtered.groupby(["sample_name", "read_id"])
        .agg(
            {"is_brdu": [np.sum, len], "read_length": np.mean, "pos": [np.min, np.max]}
        )
        .reset_index()
    )
    brdu_per_read.loc[:, "brdu_per_thy"] = (
        brdu_per_read.is_brdu["sum"] / brdu_per_read.is_brdu["len"]
    )
    brdu_per_read.loc[:, "brdu_per_thy_log"] = np.log2(
        brdu_per_read.brdu_per_thy + 0.001
    )
    brdu_per_read.loc[:, "labelled"] = brdu_per_read.brdu_per_thy > 0.1
    # plot
    for sample in set(brdu_per_read.sample_name):
        ax.hist(
            brdu_per_read.loc[brdu_per_read.sample_name == sample, "brdu_per_thy_log"],
            histtype="step",
            density=True,
            lw=3,
            bins=50,
    ...
    ax.set(
        title="Distribution of BrdU-percentage per read [log2]",
        xlabel="Brdu/Thymidine [log2]",
        ylabel="density",
    )
    plt.legend()
...
def plot_fraction_labelled(label_frame, ax):
    sbn.barplot(
        x="sample", y="Label fraction", data=label_frame, ax=ax, color="tab:blue"
    )
...
label_perc = {
    Path(path).name.split("_")[1]: percentage_labelled_reads(path)
    for path in args.label_library_paths
}
label_frame = (
    pd.DataFrame(label_perc, index=["Label fraction"])
    .T.reset_index()
...
count_frame = (
    pd.DataFrame(read_count).T.reset_index().rename(columns={"index": "sample"})
)
count_frame_molten = pd.melt(count_frame, id_vars="sample").rename(
    columns={"variable": "contact_type", "value": "read_number"}
)
