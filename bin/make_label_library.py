...
class BrdURead:
    ...
    def is_labelled(self, prob_cutoff, brdu_cutoff):
        ...
        if number_thy == 0:
            return False
        number_labelled_thy = len(
            [i for i in self._thymidines if i["prob_brdu"] > prob_cutoff]
        )
    ...
    for line_number, line in enumerate(f):
        ...
        if line[0] == ">":
            ...
            if current_read is not None:
                alignment_index[current_read.get_id()] = current_read.is_labelled(
                    prob_cutoff, brdu_cutoff
                )
...
parser = argparse.ArgumentParser(
    description="Create label library containing mapping between read_id and labeling state"
)
