...
def assign_chunk(df, label_lib):
    index_1 = df.read_name.str.cat(
        [df.align1_chrom, df.align1_start.astype("str"), df.align1_end.astype("str")],
        sep="_",
    )
    index_2 = df.read_name.str.cat(
        [df.align2_chrom, df.align2_start.astype("str"), df.align2_end.astype("str")],
        sep="_",
    )
    label_1 = index_1.apply(lambda index: label_lib.get(index, None))
    label_2 = index_2.apply(lambda index: label_lib.get(index, None))
    return pd.DataFrame({"label_1": label_1, "label_2": label_2})


# parse args

parser = argparse.ArgumentParser(
    description="Assign a read to be labelled or unlabelled in the output pairs file depending on the labelling state"
)
...
pair_frame_clean.to_csv(
    out_stream,
    sep="\t",
    columns=list(pair_frame_clean.columns),
    header=False,
    index=False,
)
