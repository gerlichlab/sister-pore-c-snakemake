...
parser = argparse.ArgumentParser(
    description="Transfer weights from cis_and_trans to cis and trans"
)
...
for binSize in parsed_resolutions:
    print(
        f"    Transferring from {args.input_cis_and_trans} to { args.output_cis} for binsize {binSize}"
    )
    transferWeights(args.input_cis_and_trans, args.output_cis, binSize)
    print(
        f"    Transferring from {args.input_cis_and_trans} to { args.output_trans} for binsize {binSize}"
    )
