rule generate_brdu_index:
    ...
    # container: "library://mboemo/dnascent/dnascent:4.0.2"
    log:
        to_log(paths.brdu_calling.index)
    shell:
        "singularity run library://mboemo/dnascent/dnascent:4.0.2 index -f {input.fast5} -s {input.seq_summary} -o {output}"
